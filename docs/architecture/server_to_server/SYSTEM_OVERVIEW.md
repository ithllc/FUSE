...
- **Deep Health Check** (`/health`): Verifies Redis connectivity (with latency), all 6 component initializations (Gemini Live, Vision, Pro, Diagram Renderer, Imagen, Veo3), and session state summary including recent error events. Video streaming state is reported via WebSocket `video_status` messages rather than the health endpoint (issue #24).
